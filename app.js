...
function renderReader(master,mode){
  ...
  const sourceKey=canonicalKey(s.key); const targetKey=canonicalKey($("transposeKeySelect")?.value)||sourceKey;
...
function keyTestProgression(song,targetKey){const sourceKey=canonicalKey(song.key)||canonicalKey(targetKey)||"C";const target=canonicalKey(targetKey)||sourceKey;const chords=chordTokens(song.chords);return (chords.length?chords:[sourceKey]).slice(0,4).map(c=>transposeChord(c,sourceKey,target));}
...
async function saveTransposedCopy(){
  const master=masterSongs.find(s=>s.id===readingId);if(!master)return;const current=dataForSong(master,readingMode);const sourceKey=canonicalKey(current.key),targetKey=canonicalKey($("transposeKeySelect")?.value);if(!sourceKey||!targetKey)return alert("Set a valid song key first.");
