...
const ROLES = ["Worship Leader","Singers","Electric Guitar","Acoustic Guitar","Bass Guitar","Keyboard","Drum"];
...
let cueDismissTimer=null;
let liveModeListId=localStorage.getItem("bandaidLiveModeListId")||"";
let liveModeIndex=0;
let liveModeFontScale=Math.max(.75,Math.min(1.8,Number(localStorage.getItem("bandaidLiveModeFontScale"))||1.1));
let cueChannel = null;
...
function roleSlug(role){
  return ({"Worship Leader":"worship-leader","Singers":"singer","Electric Guitar":"electric-guitar","Acoustic Guitar":"acoustic-guitar","Bass Guitar":"bass-guitar","Keyboard":"keyboard","Drum":"drum"})[role] || "singer";
}
...
function requireUser(){
  if(!transportUser) throw new Error("Secure BandAid connection is not ready.");
...
function makeDiagram(name,fretsRaw,fingersRaw){
  ...
  return `<span class="shape-label">${esc(name||"")}</span>
    <div class="saved-chord-grid-wrap">
      ...
      <div class="saved-chord-fretboard" aria-label="${esc(name||"Chord")} fretboard">${dots}<span class="saved-base-fret-corner">${minFret}fr</span></div>
    </div>
...
function renderLiveSessionUI(){
  ...
  if(liveSession){$("openLiveModeBtn")?.classList.remove("hidden");$("connectedSessionCode").textContent=liveSession.code||"LIVE";$("liveSessionTitle").textContent="Band connected";$("liveSessionHelp").textContent=isLeader?"Your cue buttons will broadcast to this session.":"Worship Leader cues will appear on this device.";setSessionStatus(`Connected as ${activeRole}.`,"live");}
  else{$("openLiveModeBtn")?.classList.add("hidden");$("liveSessionTitle").textContent=isLeader?"Start a live session":"Join the live session";$("liveSessionHelp").textContent=isLeader?"Create a short session code and share it with the band.":"Enter the session code shown by your Worship Leader.";setSessionStatus("Not connected.");}
...
async function leaveLiveSession(){clearLiveCue();closeLiveChordMode();stopLeaderHeartbeat();const s=liveSession;if(!s){saveLiveSession(null);return;}try{requireUser();const {data,error}=await supabaseClient.rpc("leave_band_session",{target_session:s.id});if(error)throw error;if(data==="ended")setSessionStatus("Live session ended for everyone.","error");}catch(err){setSessionStatus(`Could not leave session: ${err.message}`,"error");return;}if(realtimeChannel){try{await supabaseClient.removeChannel(realtimeChannel);}catch(e){}}realtimeChannel=null;saveLiveSession(null);}
async function sendLiveCue(label){const cue={label,sentAt:new Date().toISOString(),from:"Worship Leader"};localStorage.setItem(LIVE_CUE_KEY,JSON.stringify(cue));if(cueChannel)cueChannel.postMessage(cue);if(!liveSession)return setSessionStatus("Create a live session before sending cues.","error");try{const user=requireUser();const {error}=await supabaseClient.from("worship_cues").insert({session_id:liveSession.id,cue:label,created_by:user.id});if(error)throw error;$("lastCueSent").textContent=`Sent: ${label} · ${new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}`;qsa(".cue-btn").forEach(btn=>btn.classList.toggle("active",btn.dataset.cue===label));if(cueDismissTimer)clearTimeout(cueDismissTimer);cueDismissTimer=setTimeout(clearLiveCue,60000);}catch(err){setSessionStatus(`Cue failed: ${err.message}`,"error");}}
...
// ---------- Full-screen Live Chords ----------
function liveListItems(){if(!liveModeListId)return [];return listItemsFor(liveModeListId).map(item=>({item,resolved:songForListItem(item)})).filter(x=>x.resolved);}
function applyLiveModeFont(){if($("liveModeChords"))$("liveModeChords").style.fontSize=`${liveModeFontScale}rem`;}
function changeLiveModeFont(delta){liveModeFontScale=Math.max(.75,Math.min(1.8,Math.round((liveModeFontScale+delta)*10)/10));localStorage.setItem("bandaidLiveModeFontScale",String(liveModeFontScale));applyLiveModeFont();}
function selectLiveList(id){liveModeListId=id||"";localStorage.setItem("bandaidLiveModeListId",liveModeListId);liveModeIndex=0;renderLiveChordMode();}
function currentLiveEntry(){const items=liveListItems();if(!items.length)return null;liveModeIndex=Math.max(0,Math.min(liveModeIndex,items.length-1));return items[liveModeIndex];}
function renderLiveChordMode(){const items=liveListItems(),list=songLists.find(x=>x.id===liveModeListId);if($("liveModeSetName"))$("liveModeSetName").textContent=list?.name?`Setlist: ${list.name}`:"";$("liveModePosition").textContent=items.length?`${liveModeIndex+1} / ${items.length}`:"0 / 0";$("livePrevBtn").disabled=!items.length||liveModeIndex<=0;$("liveNextBtn").disabled=!items.length||liveModeIndex>=items.length-1;const entry=currentLiveEntry();if(!entry){$("liveModeTitle").textContent="Choose a song";$("liveModeArtist").textContent="";$("liveModeSourceKey").textContent="Key —";$("liveModeKeySelect").disabled=true;$("liveModeChords").innerHTML='<div class="live-mode-empty"><strong>No song selected</strong><span>Open Setlist to add songs for this live session.</span></div>';applyLiveModeFont();return;}const {data,master}=entry.resolved;$("liveModeTitle").textContent=master.title;$("liveModeArtist").textContent=master.artist||"";const source=inferredSongKey(data)||effectiveSongKey(data)||"C";$("liveModeSourceKey").textContent=`Key ${source}`;$("liveModeKeySelect").disabled=false;if(!$("liveModeKeySelect").dataset.touched)$("liveModeKeySelect").value=source;const target=canonicalKey($("liveModeKeySelect").value)||source,base=data.chordLayout||data.chord_layout||buildChordLayout(data.chords);renderChordLayout($("liveModeChords"),transposeLayout(base,source,target));applyLiveModeFont();}
async function openLiveChordMode(){if(!liveSession)return alert("Join or create a live session first.");try{await Promise.all([loadSongLibrary(),loadSongLists()]);}catch(e){}if(liveModeListId&&!songLists.some(x=>x.id===liveModeListId))liveModeListId="";if(!liveModeListId&&songLists.length)liveModeListId=songLists[0].id;$("liveChordMode").classList.remove("hidden");document.body.classList.add("live-mode-open");delete $("liveModeKeySelect").dataset.touched;liveModeIndex=0;renderLiveChordMode();if(!liveModeListId)openLiveSetlist();}
function closeLiveChordMode(){$("liveChordMode")?.classList.add("hidden");$("liveSetlistDialog")?.classList.add("hidden");document.body.classList.remove("live-mode-open");}
function livePrev(){if(liveModeIndex>0){liveModeIndex--;delete $("liveModeKeySelect").dataset.touched;renderLiveChordMode();$("liveModeChords").scrollTop=0;}}
function liveNext(){const items=liveListItems();if(liveModeIndex<items.length-1){liveModeIndex++;delete $("liveModeKeySelect").dataset.touched;renderLiveChordMode();$("liveModeChords").scrollTop=0;}}
async function openLiveSetlist(){try{await loadSongLists();}catch(err){return alert(`Could not load lists: ${err.message}`);}if(liveModeListId&&!songLists.some(x=>x.id===liveModeListId))liveModeListId="";if(!liveModeListId&&songLists.length)liveModeListId=songLists[0].id;renderLiveSetlist();$("liveSetlistDialog").classList.remove("hidden");}
function closeLiveSetlist(){$("liveSetlistDialog")?.classList.add("hidden");}
function renderLiveSetlist(){const select=$("liveListSelect");select.innerHTML=songLists.length?songLists.map(l=>`<option value="${safeId(l.id)}">${esc(l.name)}</option>`).join(""):'<option value="">No lists yet</option>';select.value=liveModeListId||"";const items=liveListItems();$("liveSetSongCount").textContent=`${items.length} song${items.length===1?"":"s"}`;$("liveSetRoleLabel").textContent="All songs";$("liveSetSongs").innerHTML=items.length?items.map((entry,i)=>{const {master,mode}=entry.resolved;return `<div class="live-set-song-row ${i===liveModeIndex?"current":""}"><button class="live-set-open-song" data-index="${i}" type="button"><span>${i+1}</span><div><strong>${esc(master.title)}</strong><small>${esc(master.artist||"")} · ${mode==="mine"?"My Copy":"Official"}</small></div></button><button class="ghost compact live-set-remove" data-item-id="${safeId(entry.item.id)}" type="button">Remove</button></div>`;}).join(""):'<div class="empty-mini">No songs in this set yet.</div>';const term=($("liveSetSongSearch")?.value||"").trim().toLowerCase(),candidates=masterSongs.filter(s=>[s.title,s.artist,s.role].join(" ").toLowerCase().includes(term));$("liveSetSongPicker").innerHTML=candidates.length?candidates.map(s=>`<div class="live-picker-row"><div><strong>${esc(s.title)}</strong><small>${esc(s.artist||"")}${s.role?` · ${esc(s.role)}`:""}</small></div><div class="live-picker-actions"><button class="secondary compact live-add-official" data-song-id="${safeId(s.id)}" type="button">Official</button>${personalCopies.has(s.id)?`<button class="secondary compact live-add-mine" data-song-id="${safeId(s.id)}" type="button">My Copy</button>`:""}</div></div>`).join(""):'<div class="empty-mini">No matching songs.</div>';qsa(".live-set-open-song").forEach(btn=>btn.addEventListener("click",()=>{liveModeIndex=Number(btn.dataset.index)||0;delete $("liveModeKeySelect").dataset.touched;renderLiveChordMode();closeLiveSetlist();}));qsa(".live-set-remove").forEach(btn=>btn.addEventListener("click",async()=>{await removeSongListItem(btn.dataset.itemId);await loadSongLists();renderLiveSetlist();renderLiveChordMode();}));qsa(".live-add-official").forEach(btn=>btn.addEventListener("click",()=>addSongToLiveSet(btn.dataset.songId,"official")));qsa(".live-add-mine").forEach(btn=>btn.addEventListener("click",()=>addSongToLiveSet(btn.dataset.songId,"mine")));}
async function createLiveList(){const name=$("liveNewListName").value.trim();if(!name)return;const {data,error}=await supabaseClient.rpc("bandaid_create_song_list",{p_token:accountToken,p_name:name});if(error)return alert(error.message);$("liveNewListName").value="";await loadSongLists();const madeId=data?.id||songLists.find(l=>l.name===name)?.id;if(madeId)selectLiveList(madeId);renderLiveSetlist();}
async function addSongToLiveSet(songId,version){if(!liveModeListId)return alert("Create or choose a setlist first.");const {error}=await supabaseClient.rpc("bandaid_add_song_to_list",{p_token:accountToken,p_list_id:liveModeListId,p_song_id:songId,p_version:version});if(error)return alert(error.message);await loadSongLists();renderLiveSetlist();renderLiveChordMode();}
...
$("dismissLiveCueBtn")?.addEventListener("click",clearLiveCue);
$("openLiveModeBtn")?.addEventListener("click",openLiveChordMode);
$("exitLiveModeBtn")?.addEventListener("click",closeLiveChordMode);
$("livePrevBtn")?.addEventListener("click",livePrev);
$("liveNextBtn")?.addEventListener("click",liveNext);
$("liveSetlistBtn")?.addEventListener("click",openLiveSetlist);
$("closeLiveSetlistBtn")?.addEventListener("click",closeLiveSetlist);
$("liveListSelect")?.addEventListener("change",e=>{selectLiveList(e.target.value);renderLiveSetlist();});
$("liveCreateListBtn")?.addEventListener("click",createLiveList);
$("liveNewListName")?.addEventListener("keydown",e=>{if(e.key==="Enter")createLiveList();});
$("liveSetSongSearch")?.addEventListener("input",renderLiveSetlist);
$("liveModeKeySelect")?.addEventListener("change",()=>{$("liveModeKeySelect").dataset.touched="1";renderLiveChordMode();});
$("liveFontDownBtn")?.addEventListener("click",()=>changeLiveModeFont(-.1));
$("liveFontUpBtn")?.addEventListener("click",()=>changeLiveModeFont(.1));
